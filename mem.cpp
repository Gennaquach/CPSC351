...
void tlb_add(int index, page_node entry) { 
    //entry = tlb[index++ % TLB_SIZE];
    check_tlb(index);
    if (check_tlb(index) == -1)
    {
        for(int i = 0; i < max; i++)
        {
            if (tlb[i].npage == get_page(index))
            {
                for(i = i; i < max -1; i++)
                {
                     tlb[i] = tlb[i+1];
        ...
        if (full == 0)
        {
            ...
            tlb[full].npage = get_page(index);
            tlb[full].frame_num = get_offset(index);
        }
        if (max < TLB_SIZE - 1)
        {
            max++;
...
void tlb_miss(size_t& frame, size_t& page, size_t& tlb_track) {
    ...
    for (int i = 0; i < TLB_SIZE; i++) {
        if (tlb[i].npage != page) { 
            framenum = tlb[i].npage; }
    }
 } // TODO

void fifo_replace_page(size_t& frame ) { 
    for (int i = 0; i < PTABLE_SIZE; i++) {
        if (pg_table[i].is_present == true) {
...
void check_address_value(size_t logic_add, size_t page, size_t offset, size_t physical_add,
    ...
    printf("log: %5lu 0x%04lx (pg:%3lu, off:%3lu)-->phy: %5lu (frm: %3lu) (prv: %3lu)--> val: %4d == value: %4d -- %s", logic_add, logic_add, page, offset, physical_add, frame, prev_frame, val, value, passed_or_failed(val == value));
...
void run_simulation() { 
    ...
    // double pageFaultRate = page / frames_used;
	// double TLBHitRate = tlb_hit / pg_faults;
